...
@implementation DMSDWebImageDecoder
static DMSDWebImageDecoder *sharedInstance;
...
- (void)notifyDelegateOnMainThreadWithInfo:(NSDictionary *)dict
{
    ...
    id <DMSDWebImageDecoderDelegate> delegate = [decodeInfo objectForKey:DELEGATE_KEY];
...
- (void)decodeImage:(UIImage *)image withDelegate:(id<DMSDWebImageDecoderDelegate>)delegate userInfo:(NSDictionary *)info
{
    ...
    DMSDWIRelease(operation);
}

- (void)dealloc
{
    DMSDWISafeRelease(imageDecodingQueue);
    DMSDWISuperDealoc;
}

+ (DMSDWebImageDecoder *)sharedImageDecoder
{
    if (!sharedInstance)
    {
        sharedInstance = [[DMSDWebImageDecoder alloc] init];
    }
...
+ (UIImage *)decodedImageWithImage:(UIImage *)image
{
    ...
    return DMSDWIReturnAutoreleased(decompressedImage);
}
