...
static DMSDWebImageManager *instance;
...
- (void)dealloc
{
    DMSDWISafeRelease(downloadDelegates);
    DMSDWISafeRelease(downloaders);
    DMSDWISafeRelease(cacheDelegates);
    DMSDWISafeRelease(cacheURLs);
    DMSDWISafeRelease(downloaderForURL);
    DMSDWISafeRelease(failedURLs);
    DMSDWISuperDealoc;
}
...
+ (id)sharedManager
{
    if (instance == nil)
    {
        instance = [[DMSDWebImageManager alloc] init];
    }
...
{
    return [[DMSDImageCache sharedImageCache] imageFromKey:[self cacheKeyForURL:url]];
}

/*
 * @deprecated
 */
- (void)downloadWithURL:(NSURL *)url delegate:(id<DMSDWebImageManagerDelegate>)delegate retryFailed:(BOOL)retryFailed
{
 ...
 * @deprecated
 */
- (void)downloadWithURL:(NSURL *)url delegate:(id<DMSDWebImageManagerDelegate>)delegate retryFailed:(BOOL)retryFailed lowPriority:(BOOL)lowPriority
{
...
- (void)downloadWithURL:(NSURL *)url delegate:(id<DMSDWebImageManagerDelegate>)delegate
{
    [self downloadWithURL:url delegate:delegate options:0];
}

- (void)downloadWithURL:(NSURL *)url delegate:(id<DMSDWebImageManagerDelegate>)delegate options:(SDWebImageOptions)options
{
    [self downloadWithURL:url delegate:delegate options:options userInfo:nil];
}

- (void)downloadWithURL:(NSURL *)url delegate:(id<DMSDWebImageManagerDelegate>)delegate options:(SDWebImageOptions)options userInfo:(NSDictionary *)userInfo
{
    ...
    NSDictionary *info = [NSDictionary dictionaryWithObjectsAndKeys:
                          ...
                          nil];
    [[DMSDImageCache sharedImageCache] queryDiskCacheForKey:[self cacheKeyForURL:url] delegate:self userInfo:info];
}
...
- (void)downloadWithURL:(NSURL *)url delegate:(id)delegate options:(SDWebImageOptions)options userInfo:(NSDictionary *)userInfo success:(void (^)(UIImage *image))success failure:(void (^)(NSError *error))failure
{
    ...
    NSDictionary *info = [NSDictionary dictionaryWithObjectsAndKeys:
                          ...
                          failureCopy, @"failure",
                          nil];
    DMSDWIRelease(successCopy);
    DMSDWIRelease(failureCopy);
    [[DMSDImageCache sharedImageCache] queryDiskCacheForKey:[self cacheKeyForURL:url] delegate:self userInfo:info];
}
#endif

- (void)cancelForDelegate:(id<DMSDWebImageManagerDelegate>)delegate
{
    NSUInteger idx;
    ...
    {
        DMSDWebImageDownloader *downloader = DMSDWIReturnRetained([downloaders objectAtIndex:idx]);
        ...
        DMSDWIRelease(downloader);
    }
}

#pragma mark SDImageCacheDelegate

- (NSUInteger)indexOfDelegate:(id<DMSDWebImageManagerDelegate>)delegate waitingForURL:(NSURL *)url
{
...
- (void)imageCache:(DMSDImageCache *)imageCache didFindImage:(UIImage *)image forKey:(NSString *)key userInfo:(NSDictionary *)info
{
    ...
    id<DMSDWebImageManagerDelegate> delegate = [info objectForKey:@"delegate"];
...
- (void)imageCache:(DMSDImageCache *)imageCache didNotFindImageForKey:(NSString *)key userInfo:(NSDictionary *)info
{
    ...
    DMSDWebImageDownloader *downloader = [downloaderForURL objectForKey:url];

    if (!downloader)
    {
        downloader = [DMSDWebImageDownloader downloaderWithURL:url delegate:self userInfo:info lowPriority:(options & SDWebImageLowPriority)];
...
- (void)imageDownloader:(DMSDWebImageDownloader *)downloader didUpdatePartialImage:(UIImage *)image
{
    ...
    for (NSInteger idx = (NSInteger)[downloaders count] - 1; idx >= 0; idx--)
    {
        ...
        DMSDWebImageDownloader *aDownloader = [downloaders objectAtIndex:uidx];
        if (aDownloader == downloader)
        {
            id<DMSDWebImageManagerDelegate> delegate = [downloadDelegates objectAtIndex:uidx];
            DMSDWIRetain(delegate);
            DMSDWIAutorelease(delegate);
...
- (void)imageDownloader:(DMSDWebImageDownloader *)downloader didFinishWithImage:(UIImage *)image
{
    ...
    if (image)
    {
        ...
        [[DMSDImageCache sharedImageCache] storeImage:image
