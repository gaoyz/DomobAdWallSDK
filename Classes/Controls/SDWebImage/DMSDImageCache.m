...
static DMSDImageCache *instance;

@implementation DMSDImageCache
...
- (id)init
{
    if ((self = [super init]))
    {
        ...
        diskCachePath = DMSDWIReturnRetained([[paths objectAtIndex:0] stringByAppendingPathComponent:@"ImageCache"]);
...
- (void)dealloc
{
    DMSDWISafeRelease(memCache);
    DMSDWISafeRelease(diskCachePath);
    DMSDWISafeRelease(cacheInQueue);
...
+ (DMSDImageCache *)sharedImageCache
{
    if (instance == nil)
    {
        instance = [[DMSDImageCache alloc] init];
    }
...
- (void)storeKeyWithDataToDisk:(NSArray *)keyAndData
{
    ...
    else
    {
        ...
        UIImage *image = DMSDWIReturnRetained([self imageFromKey:key fromDisk:YES]); // be thread safe with no lock
        if (image)
        {
#if TARGET_OS_IPHONE
    ...
    DMSDWIRelease(fileManager);
}

- (void)notifyDelegate:(NSDictionary *)arguments
{
    NSString *key = [arguments objectForKey:@"key"];
    id <DMSDImageCacheDelegate> delegate = [arguments objectForKey:@"delegate"];
...
{
    NSString *key = [arguments objectForKey:@"key"];
    NSMutableDictionary *mutableArguments = DMSDWIReturnAutoreleased([arguments mutableCopy]);

    UIImage *image = DMSDScaledImageForPath(key, [NSData dataWithContentsOfFile:[self cachePathForKey:key]]);
...
- (void)storeImage:(UIImage *)image imageData:(NSData *)data forKey:(NSString *)key toDisk:(BOOL)toDisk
{
    ...
    if (toDisk)
    {
        ...
        NSInvocationOperation *operation = DMSDWIReturnAutoreleased([[NSInvocationOperation alloc] initWithTarget:self
...
- (UIImage *)imageFromKey:(NSString *)key fromDisk:(BOOL)fromDisk
{
    ...
    if (!image && fromDisk)
    {
        image = DMSDScaledImageForPath(key, [NSData dataWithContentsOfFile:[self cachePathForKey:key]]);
        if (image)
        {
            [memCache setObject:image forKey:key];
        }
...
- (void)queryDiskCacheForKey:(NSString *)key delegate:(id <DMSDImageCacheDelegate>)delegate userInfo:(NSDictionary *)info
{
    if (!delegate)
    {
        return;
